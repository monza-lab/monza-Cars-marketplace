...
- BeForward catalog coverage requires the `?mode=coverage` cron (scheduled separately in `vercel.json`); the default `fresh` mode intentionally re-scans page 1 for newest listings and does not advance the DB coverage cursor.
