...
- The email-only Haus Report funnel must accept returning leads and email addresses already present in `user_credits`; account existence and prior lead reports do not trigger an auth/claim gate, while the request rate limit remains the abuse boundary. Its CTA must invoke `/api/analyze/v3` with the report-scoped token and may unlock/email only after all ten V3 sections and final synthesis complete; never substitute legacy `/api/analyze`.
