...
- The email-only Haus Report funnel grants one anonymous completed report per durable device/email identity; claimed-account emails require authentication and report attempts 2+ require account claim. Reserve access atomically in Postgres across email, device, and hourly IP/device limits, expire abandoned pending reservations, preserve ready report links, and release pending access on every post-reservation failure. Its CTA must invoke `/api/analyze/v3` with the report-scoped token and may unlock/email only after all ten V3 sections and final synthesis complete; never substitute legacy `/api/analyze`. Report access tokens are server-validated capabilities and must never enter analytics payloads or query-bearing CAPI URLs. User-facing funnel copy must describe the three free reports as a one-time allowance: the first by email and the remaining two after account claim.
- Vitest must pin `process.env.TZ = "UTC"` in `vitest.config.ts` because report fixtures use date-only UTC values and otherwise render the previous calendar day in negative-offset timezones.
- V3 premium market evidence must fall back to strict Supabase HTTP listing reads when direct Postgres is unavailable, reject title/model contradictions before valuation, and use the median from at least three clean strict comparables ahead of broader series or family aggregates. Broader aggregates are only the baseline fallback when the strict set has fewer than three usable prices.
- Classic.com Porsche scraper runs must filter discovered vehicle summaries to the requested make and normalizable/year-bearing rows before counting, normalizing, or writing; broad search false positives and yearless summaries are coverage skips, not scraper errors.
- Cron scraper routes must derive `success` from real errors and never hardcode `success: true`. Filter soft errors (`Skipped:`, 404/410) from hard errors (403/429/5xx/timeout/failed/circuit-break, plus DB write/constraint failures), set `success = no hard errors`, keep `errors_count`/`error_messages` populated so health shows `degraded`, and return `status: success ? 200 : 500`. Mirror `porsche`/`classic` routes; `autoscout24`/`ferrari`/`elferspot` were the offenders.
- Adding a value to `ScraperName` (monitoring/types.ts) requires updating every exhaustive `Record<ScraperName,…>` literal in `src/app/[locale]/admin/scrapers/ScrapersDashboardClient.tsx` (labels, runtime, cadence + the names array) and the `JOB_SPECS` list in `scripts/scraper-health-audit.ts`; otherwise tsc breaks and the job is invisible to the health audit. Use cadence `daily` only when the job is scheduled in `vercel.json`, else `external`.
- Any collector that maps `any`-typed scraper output into a row must reject missing/empty `url` before `canonicalizeUrl` (which returns its input on parse failure) to avoid null `source_url` NOT-NULL upsert violations. Ferrari's `normalizeFromBaseAndUrl` guards this.
- Scraper assurance repair must propagate `--no-lifecycle-mutations` through `run-scrapers.ts`; every transitively invoked lifecycle-capable job must honor the guard (AutoTrader uses `--noDelist`), and regression tests must verify the transitive command path rather than only the top-level command text.
- AutoScout24's stale-delist (`refreshStaleListings`) must be gated on shard coverage (skip below ~50% of `shardsCompleted/shardsTotal`); the collector's `/tmp` checkpoint is ephemeral on Vercel so a run may cover only a fraction of shards, and delisting on a partial run false-delists live cars.
- BeForward catalog coverage requires the `?mode=coverage` cron (scheduled separately in `vercel.json`); the default `fresh` mode intentionally re-scans page 1 for newest listings and does not advance the DB coverage cursor. Summary-only BeForward discovery must preserve each valid stock-list CDN thumbnail so image coverage does not depend on bounded detail enrichment. BeForward field repair must run `scripts/bf-enrich-cli.ts` through its persistent Scrapling session without depending on an app server, and a nonempty queue with zero enriched rows must fail the repair job.
- AutoTrader full inventory runs must use the disjoint price shards because its gateway rejects pages above 100; verify shard totals equal the unfiltered total before writing, and classify delisting from the canonical listing page's explicit `expired-ad=true` redirect because the retired product-page API returns false 404s for live adverts.
- CollectingCars listing discovery must accept card-selector matches only when they contain a `/cars/`, `/lots/`, or `/for-sale/` detail link; CSS-module names can place `listing-tile` on nested watch icons, so use the link-derived listing container fallback and preserve a live-DOM regression fixture.
