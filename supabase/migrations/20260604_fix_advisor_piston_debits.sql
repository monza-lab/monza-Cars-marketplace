-- Fix advisor Pistons debits.
-- The previous Pistons economy RPC wrote advisor usage rows with amount = 0
-- and did not reduce user balances. Advisor usage should debit the same
-- subscription-first, pack-second balance as reports. The unlimited_reports
-- flag applies only to REPORT_USED.
...
BEGIN
  ...
  IF p_supabase_user_id IS NOT NULL THEN
    ...
    ELSE
      ...
      IF v_total_balance < p_amount AND NOT (p_type = 'REPORT_USED' AND COALESCE(v_unlimited, false)) THEN
        RAISE EXCEPTION 'insufficient_credits';
      END IF;

      IF NOT (p_type = 'REPORT_USED' AND COALESCE(v_unlimited, false)) THEN
      ...
      v_signed_amount := -p_amount;
      IF p_type = 'REPORT_USED' AND COALESCE(v_unlimited, false) THEN
        v_signed_amount := 0;
      END IF;
    END IF;
  ELSE
