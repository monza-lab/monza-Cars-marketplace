        ...
        {!hideSearch && (
          <div className="relative mb-3" ref={inputRef}>
            ...
            {searchQuery && (
              <button
                onClick={clearSearch}
                className="absolute right-3 top-1/2 -translate-y-1/2 size-5 rounded-full bg-foreground/10 hover:bg-foreground/20 flex items-center justify-center transition-colors"
              >
            ...
            <AnimatePresence>
              {showSuggestions && suggestions.length > 0 && (
                <motion.div
                  ...
                  className="absolute top-full left-0 right-0 mt-1 bg-background/98 backdrop-blur-xl border border-border rounded-xl overflow-hidden z-50 shadow-2xl"
                >
                  {suggestions.map((variant, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full px-4 py-2.5 text-left text-[11px] text-foreground hover:bg-primary/8 transition-colors border-b border-border last:border-b-0"
                      >
                        {isFullName ? (
                          <>
                            Porsche <span className="text-primary">{variant}</span>
                          </>
                        ) : (
                          <>
                            {familyName} <span className="text-primary">{variant}</span>
                          </>
                        )}
                      </button>
                    )
