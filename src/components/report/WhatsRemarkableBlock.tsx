...
export function WhatsRemarkableBlock({
  ...
  const subtitle =
    tier === "tier_1"
      ? `${claims.length} ${claims.length === 1 ? "finding" : "findings"} about this specific VIN` // [HARDCODED]
      : tier === "tier_2"
        ? `${claims.length} findings with specialist context` // [HARDCODED]
        : `${claims.length} findings with specialist variant analysis` // [HARDCODED]
    ...
    <section className="px-4 py-6" aria-labelledby="remarkable-heading">
      ...
      {claims.length > 0 ? (
        ...
        <p className="mt-4 rounded-xl border border-dashed border-border bg-card/30 p-4 text-[13px] text-muted-foreground">
          {/* [HARDCODED] */}No remarkable findings were extracted for this listing.
        </p>
      )}
