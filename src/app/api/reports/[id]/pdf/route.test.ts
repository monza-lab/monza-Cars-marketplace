...
vi.mock("@/lib/reports/reportSections", () => ({
  fetchReportSections: vi.fn(async () => []),
}))
...
vi.mock("@/lib/exports/pdf/renderReport", () => ({
  renderReportToPdfBuffer: vi.fn(async () => Buffer.from("pdf")),
}))
...
vi.mock("@/lib/reports/assembleV3Report", () => ({
  assembleV3ReportFromSections: vi.fn(() => null),
}))

import { GET } from "./route"

describe("PDF report export route access", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it("returns 401 before export work when the user is unauthenticated", async () => {
    ...
    const response = await GET(new Request("https://test.local/api/reports/live-1/pdf"), {
