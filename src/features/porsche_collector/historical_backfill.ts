 ...
 *
 * Usage:
 *   npx tsx src/features/porsche_collector/historical_backfill.ts
 */
...
async function discoverFromModelPage(
  ...
  const pageUrl = `https://bringatrailer.com/porsche/${slug}/`;
  ...
  for (const item of items) {
    ...
    if (!/porsche/i.test(item.title)) continue;
...
export async function runHistoricalBackfill(): Promise<BackfillRunResult> {
  ...
  // Get every distinct Porsche model from the DB (not just active)
  ...
  for (const model of models) {
    ...
    console.log(`[${model}] Discovering from bringatrailer.com/porsche/${slug}/ ${SLUG_OVERRIDES[model] ? "(override)" : "(auto-derived)"} ...`);
    ...
    for (const listing of listings) {
      ...
      try {
        ...
        const priceStr = normalized.pricing.hammerPrice
          ...
          : "no price";
        console.log(
          `  + ${year} Porsche ${model} — ${priceStr} (${effectiveSaleDate}) [quality: ${normalized.dataQualityScore}/100${detailScraped ? ", enriched" : ""}]`,
        );
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
 ...
 *
 * - Queries DB for every distinct Porsche model (fully dynamic)
...
export async function runLightBackfill(config: {
  ...
  // Query DB for every distinct Porsche model
  const { data: modelRows } = await readClient
    .from("listings")
    .select("model")
    .eq("make", "Porsche");
  ...
  for (const model of allModels) {
    ...
    for (const listing of inWindow) {
      ...
      try {
        ...
        written++;
        writtenForModel++;
        console.log(`  [light-backfill] + ${year} Porsche ${model} — ${saleDate}`);
      } catch (err) {
