...
describe("porsche_collector supabase mapping", () => {
  const listing: NormalizedListing = {
    source: "BaT",
    sourceId: "bat-2003-porsche-911-turbo",
  ...
  it("maps NormalizedListing into listings row shape", () => {
    ...
    expect(row.source_id).toBe("bat-2003-porsche-911-turbo");
    expect(row.source_url).toBe("https://bringatrailer.com/listing/2003-porsche-911-turbo/");
  ...
  it("maps new Auction-model aligned columns", () => {
    ...
    expect(row.title).toBe("2003 Porsche 911 Turbo");
  ...
  it("persists deterministic rarity fields for an explicit rare Porsche listing", () => {
    ...
    expect(row.rarity_score_version).toBe("listing-rarity-v8.1");
