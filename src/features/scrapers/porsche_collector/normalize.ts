...
const PORSCHE_WORD = /\bporsche\b/i;
const DISALLOWED_TITLE_WORDS = [
  // Brand-agnostic disallow list for obvious non-car or replica listings
  /\breplica\b/i,
  /\btribute\b/i,
  /\bkit\s*car\b/i,
  /\brebody\b/i,
  /\bporsche-powered\b/i,
  /\bposter\b/i,
  /\bmodel\s*car\b/i,
  /\btoy\b/i,
  /\bwheel(s|\s*set)?\b/i,
  /\bfuchs\s*wheels?\b/i,
  /\bengine\s+(only|and|&)\b/i,
  /\btype\s+911\/\d+\b/i,
  /\btransaxle\b/i,
  ...
  /\bscale\s*model\b/i,
  /\bmemorabilia\b/i,
  /\bporsche\s+(?:movie|film|documentary)\b/i,
  /\bmanuals?\b/i,
  /\bparts?\s+manuals?\b/i,
  /\boperating\s+.*manuals?\b/i,
  /\bmagazine(s)?\b/i,
